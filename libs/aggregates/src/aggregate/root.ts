...
import { EventNames, EventTypes, IEventHandlersCollection, IReadOnlyEventsCollection } from '../events/interfaces'
...
> extends AggregateBaseState<State, Events, StateValidationSchema> {
  ...
  override async save (repositoryOrSaveFunction: IRepository<State> | IRepository<State>['save']): AsyncResult<string, UnknownError | InvalidStateError | ItemNotFoundOnStorageError> {
    if (this._transactionEvents !== undefined) {
      throw new Error('Cannot save aggregate while another save is in progress')
    }
  ...
  addEventsToCurrentTransaction (events: IReadOnlyEventsCollection<Events>): void {
    if (this._transactionEvents === undefined) {
      throw new Error('Cannot add events to current transaction while no transaction is in progress')
    }
  ...
  protected async dispatchEmittedEvents (): Promise<void> {
    if (this._transactionEvents === undefined) {
      throw new Error('Cannot dispatch events while no transaction is in progress')
    }
  ...
  private async dispatchEvents (events: Array<IReadOnlyEventsCollection<Events>>): Promise<void> {
  ...
  private async dispatchEvent (event: EventTypes<Events>): Promise<void> {
    for (const handler of this._eventHandlers.get(event.type as EventNames<Events>)) {
      await handler(event)
    }
  }
