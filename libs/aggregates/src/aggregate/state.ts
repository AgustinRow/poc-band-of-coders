...
export type CompleteState<State extends IState> = State & {
  id: string;
  [isDeleted]: boolean;
  [isNew]: boolean;
};

export type PersistedState<T extends { id?: any } | { id: any }> = Omit<
  T,
  "id"
> &
  Required<Pick<T, "id">>;
...
export interface IRepositoryClient<State extends IState> {
  save: (
    aggregate: CompleteState<State>
  ) => AsyncResult<State, UnknownError | NotFoundError>;
}
...
> extends AggregateBaseEvents<Events> {
  ...
  private isKeyOfSchema(key: string): key is Extract<keyof Schema, string> {
    return (
      ...
      Object.keys(this.schema.fields).includes(key)
    );
  }

  public setField<F extends Extract<keyof Omit<State, "id">, string>>(
    field: F,
    value: Omit<CompleteState<State>, "id">[F]
  ...
  async validateState(): AsyncResult<void, InvalidStateError> {
    ...
    return validation.isOk()
      ? ok()
      : err(new InvalidStateError("Invalid state", validation.error));
  }
  ...
  async save(
    ...
    if (stateValidation.isErr()) {
      return err(stateValidation.error);
    }
  ...
  get id(): string {
    if (this._state.id === undefined) {
      throw new Error("You are trying to read an undefined id");
    }
