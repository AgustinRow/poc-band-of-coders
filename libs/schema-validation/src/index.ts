...
export function validateAt<T, K extends Extract<keyof T, string>, S>(
  schema: SchemaOf<T>,
  path: K,
  value: S
): Result<void, InvalidObjectSchemaError> {
  try {
    ...
    return err(
      new InvalidObjectSchemaError(
        "The field that you verify does not satisfied the validation schema",
        error,
        path
      )
    );
...
export async function validate<T, S>(
  schema: SchemaOf<T>,
  input: S
): AsyncResult<void, InvalidObjectSchemaError> {
  try {
