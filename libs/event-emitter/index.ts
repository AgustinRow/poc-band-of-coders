...
type EventListener<TData> = (data: TData) => AsyncResult<void, unknown> | Promise<void> | void
interface EventTypesBase { [name: string]: any }

type ValueTypes<ObjectType extends EventTypesBase & Partial<{ _listenerAdded: any, _listenerRemoved: any}>> = ObjectType[keyof ObjectType]
...
type AllEventTypes<EventTypes extends EventTypesBase> = EventTypes & {
  _listenerAdded: ValueTypes<{ [EventName in keyof EventTypes]: ListenerEventData<EventTypes, EventName> }>
  _listenerRemoved: ValueTypes<{ [EventName in keyof EventTypes]: ListenerEventData<EventTypes, EventName> }>
}
...
export class EventEmitter<EventTypes extends EventTypesBase, _EventTypes extends AllEventTypes<EventTypes> = AllEventTypes<EventTypes> > {
  private readonly _listeners = new Map<keyof _EventTypes, Set<EventListener<ValueTypes<_EventTypes>>>>()
  ...
  protected async emit<EventName extends keyof _EventTypes> (
    name: EventName,
    ...options: (
      _EventTypes[EventName] extends undefined
